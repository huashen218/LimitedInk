...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask, head_mask=None):
        ...
        scores = torch.matmul(q, k.transpose(2, 3))  # (bs, n_heads, q_length, k_length) = torch.Size([3, 12, 512, 512])
        # mask size: (bs, sent_length) torch.Size([3, 512])
        

        mask = (mask == 0).view(mask_reshp).expand_as(scores)  # (bs, n_heads, q_length, k_length)  = torch.Size([3, 12, 512, 512])
...
class DistilBertForTokenClassification(DistilBertPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None,
        ...
        sequence_output = outputs[0]  # shape = torch.Size([16, 512, 768])
        ...
        logits = self.classifier(sequence_output)  # shape = torch.Size([16, 508, 2])
